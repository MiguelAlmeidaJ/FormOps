...
$groupsSql = 'SELECT id, name, color FROM form_groups WHERE tenant_id = ?';
...
<section class="forms-hero">
    <div>
        <span class="forms-kicker">Gestão de formulários</span>
        <h1>Formulários</h1>
        <p>Gerencie inscrições, pesquisas e outros formulários.</p>
    </div>

    <?php if (canManageTenantData()): ?>
        <a href="<?= htmlspecialchars(appUrl('form-create', $selectedGroupId ? ['group_id' => $selectedGroupId] : [])) ?>" class="forms-primary-action">+ Novo formulário</a>
    <?php endif; ?>
</section>
...
<section class="forms-filter-card">
    <div>
        <h2>Organização por grupo</h2>
        <p>Filtre os formulários por área, departamento ou pasta.</p>
    </div>
    ...
    <?php if ($forms): ?>
        <div class="table-responsive">
            <table class="table forms-table align-middle mb-0">
                <thead><tr><th>Título</th><th>Grupo</th><th>Tipo</th><th>Status</th><th>Criado em</th><th class="text-end">Ações</th></tr></thead>
                <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if (!empty($form['group_name'])): ?>
                                    <span class="forms-group-badge" style="--group-color: <?= htmlspecialchars($form['group_color'] ?: '#212121') ?>"><?= htmlspecialchars($form['group_name']) ?></span>
                                <?php else: ?>
                            ...
                            <td>
                                <div class="form-actions">
                                    ...
                                    <?php if (canManageTenantData()): ?>
                                        <form method="post" action="<?= htmlspecialchars(appUrl('form-toggle-status')) ?>" data-confirm="Tem certeza que deseja <?= $form['is_active'] ? 'inativar' : 'ativar' ?> este formulário?">
                                    ...
                                    <?php if ($canCompleteForms && !isFormCompleted($form)): ?>
                                        <form method="post" action="<?= htmlspecialchars(appUrl('form-mark-completed')) ?>" data-confirm="Marcar este formulário como concluído? Ele não aceitará novas respostas e a lista de presença será liberada.">
                                            <input type="hidden" name="form_id" value="<?= (int) $form['id'] ?>">
                                    ...
                                    <?php endif; ?>
                                    <?php if (isSuperAdmin()): ?>
                                        <form method="post" action="<?= htmlspecialchars(appUrl('form-reset-responses')) ?>" data-confirm="Tem certeza que deseja resetar todas as respostas deste formulário? Essa ação não poderá ser desfeita.">
                                            <input type="hidden" name="form_id" value="<?= (int) $form['id'] ?>">
                                    ...
                                    <?php if ($canDeleteForms): ?>
                                        <form method="post" action="<?= htmlspecialchars(appUrl('form-delete')) ?>" data-confirm="Tem certeza que deseja apagar este formulário? Todas as respostas e campos também serão removidos. Essa ação não poderá ser desfeita.">
                                            <input type="hidden" name="form_id" value="<?= (int) $form['id'] ?>">
...
<div class="modal fade" id="qrCodeModal" tabindex="-1" aria-labelledby="qrCodeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content qr-modal-content">
            <div class="modal-header"><div><h2 class="modal-title" id="qrCodeModalLabel">QR Code do formulário</h2><p class="qr-modal-subtitle" id="qrCodeModalSubtitle"></p></div><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button></div>
