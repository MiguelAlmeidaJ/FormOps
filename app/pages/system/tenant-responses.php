<?php
requireSuperAdmin();
$tenantId=filter_input(INPUT_GET,'tenant_id',FILTER_VALIDATE_INT);$formId=filter_input(INPUT_GET,'form_id',FILTER_VALIDATE_INT);
$stmt=$pdo->prepare('SELECT f.id,f.title,t.name tenant_name FROM forms f JOIN tenants t ON t.id=f.tenant_id WHERE f.id=? AND f.tenant_id=?');$stmt->execute([$formId,$tenantId]);$form=$stmt->fetch(PDO::FETCH_ASSOC);
if(!$form){http_response_code(404);require __DIR__.'/../errors/404.php';exit;}
