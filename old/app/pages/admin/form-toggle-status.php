...
try {
    if (shouldScopeTenantUserToGroup()) {
        $stmt = $pdo->prepare('UPDATE forms SET is_active = ?, updated_at = NOW() WHERE id = ? AND tenant_id = ? AND form_group_id = ?');
        $stmt->execute([$nextStatus, $formId, $tenantId, currentUserFormGroupId()]);
    } else {
