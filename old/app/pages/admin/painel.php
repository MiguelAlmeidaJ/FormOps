...
if ($scopedGroupId && $hasFormGroupColumn) {
    $totalGroups = $hasGroupsTable ? dashboardCount($pdo, 'SELECT COUNT(*) FROM form_groups WHERE tenant_id = ? AND id = ?', [$tenantId, $scopedGroupId]) : 0;
    $totalForms = dashboardCount($pdo, 'SELECT COUNT(*) FROM forms WHERE tenant_id = ? AND form_group_id = ?', [$tenantId, $scopedGroupId]);
    $activeForms = dashboardCount($pdo, 'SELECT COUNT(*) FROM forms WHERE tenant_id = ? AND form_group_id = ? AND is_active = 1', [$tenantId, $scopedGroupId]);
    $totalResponses = dashboardCount($pdo, 'SELECT COUNT(*) FROM form_responses r INNER JOIN forms f ON f.id = r.form_id AND f.tenant_id = r.tenant_id WHERE r.tenant_id = ? AND f.form_group_id = ?', [$tenantId, $scopedGroupId]);
    $responsesToday = dashboardCount($pdo, 'SELECT COUNT(*) FROM form_responses r INNER JOIN forms f ON f.id = r.form_id AND f.tenant_id = r.tenant_id WHERE r.tenant_id = ? AND f.form_group_id = ? AND DATE(r.created_at) = CURDATE()', [$tenantId, $scopedGroupId]);
} else {
...
$recentFormsSql = $hasGroupsTable && $hasFormGroupColumn
    ...
    : 'SELECT f.id, f.title, f.slug, f.is_active, f.updated_at, "Sem grupo" AS group_name, COUNT(r.id) AS responses_count
       ...
       LIMIT 6';
if ($scopedGroupId && $hasFormGroupColumn) {
    $recentFormsSql = str_replace('WHERE f.tenant_id = ?', 'WHERE f.tenant_id = ? AND f.form_group_id = ?', $recentFormsSql);
}
$stmt = $pdo->prepare($recentFormsSql);
$stmt->execute($scopedGroupId && $hasFormGroupColumn ? [$tenantId, $scopedGroupId] : [$tenantId]);
...
$latestResponsesSql = 'SELECT r.id, r.created_at, f.title AS form_title
     ...
     WHERE r.tenant_id = ?
     ' . ($scopedGroupId && $hasFormGroupColumn ? 'AND f.form_group_id = ?' : '') . '
...
<style>
    .dashboard-hero{display:flex;justify-content:space-between;align-items:flex-start;gap:20px;margin-bottom:26px}.dashboard-kicker{color:#2563eb;font-weight:800;font-size:12px;text-transform:uppercase;letter-spacing:.12em}.dashboard-title{font-size:34px;font-weight:850;letter-spacing:-.04em;color:#0f172a;margin:4px 0}.dashboard-subtitle{color:#64748b;max-width:680px;margin:0}.dashboard-primary{display:inline-flex;align-items:center;gap:8px;min-height:46px;border-radius:14px;background:linear-gradient(135deg,#2563eb,#1d4ed8);color:#fff;text-decoration:none;font-weight:800;padding:0 18px;box-shadow:0 16px 30px rgba(37,99,235,.24)}.dashboard-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:18px;margin-bottom:22px}.metric-card{background:#fff;border:1px solid #e5e7eb;border-radius:20px;padding:20px;box-shadow:0 18px 40px rgba(15,23,42,.06)}.metric-top{display:flex;justify-content:space-between;align-items:center}.metric-label{color:#64748b;font-size:13px;font-weight:800;text-transform:uppercase;letter-spacing:.06em}.metric-icon{width:42px;height:42px;border-radius:14px;display:grid;place-items:center;background:rgba(37,99,235,.10);color:#2563eb;font-size:20px;font-weight:900}.metric-value{font-size:34px;font-weight:850;color:#0f172a;margin:14px 0 2px}.metric-text{color:#64748b;font-size:14px}.dashboard-columns{display:grid;grid-template-columns:minmax(0,1.35fr) minmax(340px,.65fr);gap:20px}.dashboard-card{background:#fff;border:1px solid #e5e7eb;border-radius:22px;box-shadow:0 18px 40px rgba(15,23,42,.06);overflow:hidden}.dashboard-card-head{display:flex;justify-content:space-between;align-items:center;padding:20px 22px;border-bottom:1px solid #eef2f7}.dashboard-card-title{font-size:18px;font-weight:850;color:#0f172a;margin:0}.dashboard-table{width:100%;border-collapse:collapse}.dashboard-table th{font-size:12px;color:#64748b;text-transform:uppercase;letter-spacing:.08em;font-weight:850;padding:14px 22px;background:#f8fafc}.dashboard-table td{padding:16px 22px;border-top:1px solid #f1f5f9;vertical-align:middle}.dashboard-table tr:first-child td{border-top:0}.status-pill{display:inline-flex;align-items:center;border-radius:999px;padding:5px 10px;font-size:12px;font-weight:800}.status-pill.active{background:rgba(34,197,94,.12);color:#16a34a}.status-pill.inactive{background:rgba(100,116,139,.12);color:#475569}.dashboard-action{font-weight:800;text-decoration:none;color:#2563eb}.empty-state{padding:28px 22px;color:#64748b}.response-list{display:grid}.response-item{display:flex;justify-content:space-between;gap:14px;padding:16px 22px;border-top:1px solid #f1f5f9}.response-item:first-child{border-top:0}.response-title{font-weight:800;color:#0f172a}.response-date{color:#64748b;font-size:13px;margin-top:2px}@media(max-width:1199px){.dashboard-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.dashboard-columns{grid-template-columns:1fr}}@media(max-width:767px){.dashboard-hero{display:block}.dashboard-primary{margin-top:16px}.dashboard-grid{grid-template-columns:1fr}.dashboard-table{min-width:720px}.dashboard-card{overflow:auto}}
</style>
